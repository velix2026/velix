...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const cartWithQuantity = items.map((item: Product) => ({ ...item, quantity: 1 }));
  ...
  const updateQuantity = (productId: number, newQuantity: number) => {
    ...
    setCart(newCart);
    // حفظ في localStorage بدون quantity (نحفظ المنتجات فقط)
        ...
        {cart.length === 0 ? (
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-xl shadow-md p-6 h-fit sticky top-24">
              ...
              <button
                onClick={() => alert('جاري التوجيه لصفحة الدفع...')}
